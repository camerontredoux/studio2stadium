...
async function createUser(attrs: { username: string; email: string; withSchoolProfile?: boolean }) {
...
test.group("UploadCoachesService", (group) => {
  ...
  test("matched coach gets userId + org_membership created", async ({ assert }) => {
    const coach = await createUser({ username: "coach1", email: "coach1@usc.edu", withSchoolProfile: true });
...
ghost@ucla.edu,Ghost,Er,UCLA`;
  ...
  test("re-upload is idempotent: updates existing rows, no duplicates", async ({ assert }) => {
    await createUser({ username: "coach2", email: "coach2@usc.edu", withSchoolProfile: true });

    const csv = `email,firstName,lastName,organization
coach2@usc.edu,Coach,Two,USC`;

    const svc = new UploadCoachesService();
    const first = await svc.execute({ orgId: summit.id, eventId: event.id, uploaderId: uploader.id, fileUrl: "test://1.csv", csv });
    const second = await svc.execute({ orgId: summit.id, eventId: event.id, uploaderId: uploader.id, fileUrl: "test://2.csv", csv });
    ...
    // No duplicate rows
    const rosters = await db.select().from(eventRosters).where(eq(eventRosters.eventId, event.id));
    assert.lengthOf(rosters, 1);
  });

  test("rows with errors are counted but valid rows are still inserted", async ({ assert }) => {
    const csv = `email,firstName,lastName,organization
,Missing,Email,USC
