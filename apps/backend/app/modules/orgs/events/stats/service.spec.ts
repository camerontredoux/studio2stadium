...
async function createUser(attrs: { username: string; email: string }) {
  const [u] = await db.insert(users).values({
    username: attrs.username, email: attrs.email, displayEmail: attrs.email,
    firstName: "T", lastName: "U", password: "x", role: "user", type: "dancer", verified: true,
  }).returning();
  return u!;
}
...
test.group("EventStatsService", (group) => {
  ...
  group.each.setup(async () => {
    ...
    const [ev] = await db.insert(orgEvents).values({
      orgId: summit.id, name: "Summit 2026", startDate: "2026-06-13", endDate: "2026-06-14", isActive: true,
    }).returning();
    event = ev!;
  });
  ...
  test("returns zero counts for empty event", async ({ assert }) => {
    ...
    assert.equal(stats.coaches, 0);
    assert.equal(stats.dancers, 0);
  ...
  test("counts coaches, dancers, registered, pending correctly", async ({ assert }) => {
    ...
    const svc = new EventStatsService();
    const stats = await svc.execute(event.id);
    assert.equal(stats.coaches, 1);
    assert.equal(stats.dancers, 3);
    assert.equal(stats.registered, 2);
    assert.equal(stats.pending, 2); // 1 unregistered coach + 1 unregistered dancer
  });

  test("recentUploads returns last 5 ordered by createdAt desc", async ({ assert }) => {
    const uploader = await createUser({ username: "up", email: "up@x.co" });
    for (let i = 0; i < 6; i++) {
      await db.insert(csvUploads).values({
        eventId: event.id, type: "coach", fileUrl: `test://f${i}.csv`,
        uploadedBy: uploader.id, rowsAdded: i, rowsUpdated: 0, rowsErrored: 0,
      });
    }
    const svc = new EventStatsService();
    const stats = await svc.execute(event.id);
