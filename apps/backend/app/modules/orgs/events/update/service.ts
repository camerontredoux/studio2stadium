...
export class UpdateEventService {
  ...
  async execute(orgId: string, eventId: string, patch: Validator, auditCtx: AuditContext) {
    return this.db.withAudit(auditCtx, async (tx, audit) => {
      ...
      if (ev) {
        const isActivateToggle = patch.isActive !== undefined && before?.isActive !== patch.isActive;
        ...
        if (isActivateToggle) {
          audit.log({
            ...
            resource: "event",
            resourceId: ev.id,
            metadata: { before: { isActive: before?.isActive }, after: { isActive: ev.isActive } },
          });
        } else {
          audit.log({
            action: "update",
