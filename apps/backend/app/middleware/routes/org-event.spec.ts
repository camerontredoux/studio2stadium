...
function mockCtxWithOrg(org: any) {
  ...
  const ctx: any = {
    org,
    auth: {
      getUserOrFail: () => { throw new Error("not authed"); }
    },
    response: {
      notFound: (body: any) => { state.notFoundBody = body; },
    },
    get orgEvent() { return state.orgEvent; },
    set orgEvent(v) { state.orgEvent = v; },
    get orgRoster() { return state.orgRoster; },
    set orgRoster(v) { state.orgRoster = v; },
  };
  return { ctx, state };
}
...
test.group("OrgEventMiddleware", (group) => {
  ...
  test("404 when no active event exists", async ({ assert }) => {
    ...
    await new OrgEventMiddleware().handle(ctx, async () => { nextCalled = true; });
  ...
  test("404 when ctx.org is missing", async ({ assert }) => {
    ...
    const ctx: any = {
      org: undefined,
      auth: { getUserOrFail: () => { throw new Error("not authed"); } },
      response: { notFound: (body: any) => { state.notFoundBody = body; } },
      get orgEvent() { return state.orgEvent; },
      set orgEvent(v: any) { state.orgEvent = v; },
    };
    let nextCalled = false;
    await new OrgEventMiddleware().handle(ctx, async () => { nextCalled = true; });
    assert.isFalse(nextCalled);
