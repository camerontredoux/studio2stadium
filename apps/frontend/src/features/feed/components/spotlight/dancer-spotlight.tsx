...
import type { RecommendedDancer } from "../../types";

type MatchTier = RecommendedDancer["matchTier"];
...
export function DancerSpotlight() {
  const { data } = useSuspenseQuery(feedQueries.recommendedDancers());
    ...
    <div className="overflow-clip rounded-2xl border">
      <div className="from-brand/10 via-brand/5 to-background relative overflow-clip bg-linear-to-br p-4">
        ...
        <div className="flex justify-between gap-2">
          <h2 className="font-semibold">Dancer Spotlight</h2>
          <MatchTierBadge matchTier={recommended.matchTier} />
        </div>
        <p className="text-muted-foreground text-sm">
          Based on your program's preferences
        </p>
      </div>
      <div className="from-brand/10 via-background to-background grid grid-cols-1 gap-2 border-t bg-linear-to-tl p-2 sm:grid-cols-2 sm:gap-4 sm:p-4">
        ...
        <div className="flex flex-col gap-2 sm:gap-4">
          <p className="line-clamp-4 text-sm">
            {recommended.about ??
              "No bio available. Check out their profile for more information."}
          </p>
          <div className="flex flex-col gap-3">
            <div className="space-y-1">
              <h3 className="text-muted-foreground text-sm">Dancer Strengths</h3>
            ...
            <div className="text-muted-foreground flex flex-wrap items-center gap-x-4 gap-y-1 text-sm">
              <span className="flex items-center gap-2">
                <TargetIcon className="size-4" /> {recommended.gpa ?? "—"} GPA
              </span>
              {recommended.gradYear ? (
                <span className="flex items-center gap-2">
                  <GraduationCapIcon className="size-4" /> Class of{" "}
                  {recommended.gradYear}
                </span>
              ) : null}
            </div>
